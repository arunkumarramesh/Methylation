...
process GTCALL {
    ...
    java -jar /proj/popgen/a.ramesh/software/picard.jar CreateSequenceDictionary -R $genome -O ${genome}.dict
    ...
    cd $pair_id
    java -jar /proj/popgen/a.ramesh/software/picard.jar AddOrReplaceReadGroups -I Aligned.sortedByCoord.out.bam -O readgroup.bam -LB species -PL illumina -PU 1 -SM $pair_id
    java -jar /proj/popgen/a.ramesh/software/picard.jar MarkDuplicates -I readgroup.bam -O marked.bam -M metrics.txt
    java -jar /proj/popgen/a.ramesh/software/picard.jar BuildBamIndex -I marked.bam
    /proj/popgen/a.ramesh/software/gatk-4.3.0.0/gatk SplitNCigarReads -R $genome  -I marked.bam -O  ${pair_id}_split.bam
...
workflow {
    ...
    gtcall_ch=GTCALL(read_pairs_ch,genome_ch,gtf_ch,cpus_ch)
}
